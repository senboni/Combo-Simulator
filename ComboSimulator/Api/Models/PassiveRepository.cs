...
namespace ComboSimulator.Api.Models
{
    public class PassiveRepository : IPassiveRepository
    {
        ...
        public async Task<Passive> AddPassive(Passive chase)
        {
            var result = await passiveContext.Passives.AddAsync(chase);
            await passiveContext.SaveChangesAsync();
            return result.Entity;
        }

        public async Task<Passive> DeletePassive(long chaseId)
        {
